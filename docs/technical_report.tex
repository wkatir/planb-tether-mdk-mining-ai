...
\hypersetup{
    ...
    pdfsubject={MDK Assignment - Tether / Plan B Network 2026 (Developer Track)},
...
\begin{titlepage}
    ...
    \begin{tabular}{rl}
        ...
        \textbf{Programme:}  & Plan B Network 2026 -- Developer Track \\[0.3em]
        \textbf{Domain:}     & Bitcoin Mining, AI / ML, Data Engineering \\[0.3em]
        \textbf{Duration:}   & 3 Weeks \\[0.3em]
        \textbf{Date:}       & April 2026 \\[0.3em]
        \textbf{Repository:} & \href{https://github.com/wkatir/planb-tether-mdk-mining-ai}{\texttt{github.com/wkatir/planb-tether-mdk-mining-ai}} \\[0.3em]
    ...
    \vfill
    {\small Submitted for Plan B Network 2026 -- Developer Track}\par
\end{titlepage}
...
\begin{center}
\begin{tikzpicture}[
    stage/.style={draw, rounded corners=2pt, minimum width=1.9cm,
                  minimum height=0.75cm, align=center, font=\scriptsize},
    arr/.style={-{Stealth[length=1.8mm]}},
    node distance=0.12cm
]
    \node[stage] (hw) {Hardware};
    \node[stage, right=of hw] (tel) {Telemetry\\Pipeline};
    \node[stage, right=of tel] (feat){Feature\\Processing};
    \node[stage, right=of feat] (ai) {AI\\Controller};
    \node[stage, right=of ai] (cmd) {Command\\Execution};
    \draw[arr] (hw) -- (tel);
    \draw[arr] (tel) -- (feat);
    \draw[arr] (feat) -- (ai);
    \draw[arr] (ai) -- (cmd);
...
The PoC keeps several design simplifications intentional for scope. The RL
agent trains against a simplified reward; a production deployment would use
the full TE/ETE reward with a multi-miner joint action space.
\texttt{MiningEnv} is single-spec; a vectorised multi-spec environment is
required before heterogeneous fleets can be jointly optimised. Storage uses
DuckDB as a local-first proxy for Hyperbee; the production target is
Hyperbee with Holepunch replication. Ingestion is via Parquet; production
would subscribe to HRPC telemetry topics. The LLM is hosted on NVIDIA NIM
for demo portability, with local Ollama deployment demonstrated by a single
env var swap. Heat-reuse revenue and halving-aware strategy adjustments are
identified as next-iteration work.
...
Mining Economics} [Conference presentations]. Plan B Network, San
Salvador.
